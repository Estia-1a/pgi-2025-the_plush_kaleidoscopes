...
void stat_report(char *source_path) {
    ...
    find_max_pixel(data, width, height, nbChannels, &max_x, &max_y, &max_val);
    fprintf(f, "max_pixel (%d, %d): %d\n\n", max_x, max_y, max_val);

    // min_pixel
    int min_x, min_y, min_val;
    find_min_pixel(data, width, height, nbChannels, &min_x, &min_y, &min_val);
    ...
    for (int i = 0; i < 3; ++i) {
        int x, y, val;
        find_max_component(data, width, height, nbChannels, comps[i], &x, &y, &val);
    ...
    for (int i = 0; i < 3; ++i) {
        int x, y, val;
        find_min_component(data, width, height, nbChannels, comps[i], &x, &y, &val);
